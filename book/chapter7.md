...
>
> **当前仓库实测（RTX PRO 6000 Blackwell）**：教师轨迹完成 23/24（`aime-2016-9-I` 三次超时未采集），用 23 条 Kimi K3 原始思维链对 Qwen2.5-1.5B-Instruct 做 LoRA SFT（3 epochs，约 27 秒）后，同题三臂对照为：基线 1/24、学生 2/24、教师 23/24。配对符号检验 p=1.0，能力恢复比例仅约 4.5%，学生反思/回溯/验算行为率与基线接近，实验状态仍为 incomplete。这个结果主要受限于样本量过小和缺一条教师轨迹，它证明了训练管线能跑通，但**不能**证明“小样本 CoT 蒸馏即可显著恢复教师能力”——要得到可信结论，需要补齐缺失轨迹、扩大数据规模并换用更强基座。
>
